...
using Enterprises.Framework.Plugin.Excel.Info;
using NPOI.SS.UserModel;
using Enterprises.Framework.Plugin.Excel.Extend.CellExtends;


namespace Enterprises.Framework.Plugin.Excel.Extend.SheetExtend
{
    public static partial class SheetExtend
    {
        ...
        public static int RemoveRows(this ISheet sheet, int startRowIndex, int endRowIndex)
        {
            ...
            //删除合并区域...
            RemoveMergedRegions(sheet, startRowIndex, endRowIndex, null, null);
            //删除图片...
            RemovePictures(sheet, startRowIndex, endRowIndex, null, null);
            //删除行
            ...
            if ((endRowIndex + 1) <= sheet.LastRowNum)
            {
                ...
                MovePictures(sheet, endRowIndex + 1,null,null,null,moveRowCount:-span);
            }
            return span;
        }
        ...
        public static int CopyRows(this ISheet sheet, int startRowIndex, int endRowIndex)
        {
            ...
            //获取模板行内的合并区域
            List<MergedRegionInfo> regionInfoList =GetMergedRegionInfos(sheet, startRowIndex, endRowIndex, null,
                null);
            //复制合并区域
            ...
            //获取模板行内的图片
            List<PictureInfo> picInfoList = GetAllPictureInfos(sheet, startRowIndex, endRowIndex, null, null);
            //复制图片
            foreach (PictureInfo picInfo in picInfoList)
            {
                picInfo.MaxRow += span;
                picInfo.MinRow += span;
                AddPicture(sheet, picInfo);
            }
